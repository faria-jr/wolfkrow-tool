...
export const harnessSprints = sqliteTable('harness_sprints', {
  id: text('id').primaryKey(),
  projectId: text('project_id').notNull().references(() => harnessProjects.id, { onDelete: 'cascade' }),
...
export const harnessRounds = sqliteTable('harness_rounds', {
  id: text('id').primaryKey(),
  sprintId: text('sprint_id').notNull().references(() => harnessSprints.id, { onDelete: 'cascade' }),
...
export class HarnessPlanner {
  ...
  async plan(projectId: string, specContent: string): Promise<HarnessSprint[]> {
    ...
    let response = '';
    for await (const chunk of this.provider.query(prompt, { signal: new AbortController().signal })) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        <Metric label="Features Passed" value={`${metrics.featuresPassed}/${metrics.featuresTotal}`} />
