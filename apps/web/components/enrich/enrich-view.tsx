...
function SessionOutputPanel({ output }: { output: SessionOutputs }) {
  ...
  return (
    <div className="space-y-2 border-t px-4 pb-4 pt-3">
      {output.validator && (
        <div>
          <p className="text-muted-foreground mb-1 text-xs font-semibold">Validator output</p>
          <pre className="bg-muted whitespace-pre-wrap rounded p-2 text-xs">{output.validator}</pre>
        </div>
      )}
      {output.enricher && (
        <div>
          <p className="text-muted-foreground mb-1 text-xs font-semibold">Enricher output</p>
          <pre className="bg-muted whitespace-pre-wrap rounded p-2 text-xs">{output.enricher}</pre>
        </div>
      )}
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-4 p-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`rounded px-1.5 py-0.5 text-xs ${statusBadge(s.status)}`}>
              {s.status}
            </span>
          </div>
        ...
        <div className="flex shrink-0 gap-2">
          ...
          <button
            onClick={onValidate}
            disabled={isActive}
            className="hover:bg-accent rounded border px-3 py-1 text-xs disabled:opacity-50"
          >
            Validate
          </button>
          <button
...
function SessionList({
  ...
  if (sessions.length === 0)
    return (
      <p className="text-muted-foreground py-12 text-center text-sm">No enrich sessions yet.</p>
    );
  return (
...
async function runEnrichAction(
  ...
  setOutputs((prev) => {
    ...
    if (endpoint === 'validate' && data['output'] !== undefined)
      return { ...prev, [id]: { ...current, validator: data['output'] } };
    if (endpoint === 'enrich' && data['enriched'] !== undefined)
      return { ...prev, [id]: { ...current, enricher: data['enriched'] } };
    return prev;
  });
}
...
export function EnrichView() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      <PageHeader title="Enrich" description="Validate and enrich knowledge sessions." />
      <CreateSessionForm
        name={name}
        creating={creating}
        onChange={setName}
