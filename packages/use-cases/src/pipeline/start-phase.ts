import type { PipelinePhase, PipelinePhaseRepo, PipelineProjectRepo, PipelineStage } from '@wolfkrow/domain';
import { PipelinePhase as PipelinePhaseEntity, NotFoundError } from '@wolfkrow/domain';

export interface StartPhaseInput { projectId: string; stage: PipelineStage; }
export interface StartPhaseOutput { phase: PipelinePhase; }
...
export class StartPhaseUseCase {
  ...
  async execute(input: StartPhaseInput): Promise<StartPhaseOutput> {
    ...
    const phase = await this.phaseRepo.save(PipelinePhaseEntity.create({ projectId: project.id, stage: input.stage }).start());
    return { phase };
  }
}
