...
describe('VoiceRecorder', () => {
  ...
  it('clicking start calls startRecording', async () => {
    ...
    vi.mocked(useStt).mockReturnValue({
      isRecording: false, transcript: '', startRecording, stopRecording: vi.fn(),
      reset: vi.fn(), error: null,
    });
    render(<VoiceRecorder />);
  ...
  it('shows transcript when present', () => {
    vi.mocked(useStt).mockReturnValue({
      isRecording: false, transcript: 'recognized text', startRecording: vi.fn(),
      stopRecording: vi.fn(), reset: vi.fn(), error: null,
    });
    render(<VoiceRecorder />);
  ...
  it('shows stop button while recording and calls onTranscript', async () => {
    ...
    vi.mocked(useStt).mockReturnValue({
      isRecording: true, transcript: '', startRecording: vi.fn(),
      stopRecording, reset: vi.fn(), error: null,
    });
