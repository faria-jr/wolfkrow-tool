...
export function VaultView() {
  ...
  const [category, setCategory] = useState<'ai' | 'integration' | 'oauth' | 'other'>('ai');
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) {
      const d = await res.json() as { secrets: SecretMeta[] };
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/vault', {
        ...
        body: JSON.stringify({ key, value, displayName, category }),
      });
      if (!res.ok) {
      ...
      setShowForm(false);
      setKey(''); setValue(''); setDisplayName('');
      await load();
    } catch (err) {
            ...
            {secrets.map((s) => (
              <tr key={s.key} className="border-b px-4 last:border-0">
                ...
                <td className="px-4 py-2">
                  <span className="rounded bg-secondary px-2 py-0.5 text-xs">{s.category}</span>
                </td>
                <td className="px-4 py-2">
                  <SecretValueCell secretKey={s.key} />
                </td>
                <td className="px-4 py-2">
                  <Button size="sm" variant="destructive" onClick={() => void handleDelete(s.key)}>
                    Delete
                  </Button>
                </td>
              </tr>
      ...
      {showForm ? (
        <div className="flex flex-col gap-3 rounded border p-4">
          ...
          <div className="flex gap-2">
            <Button onClick={() => void handleSave()} disabled={saving || !key || !value || !displayName}>
              {saving ? 'Saving…' : 'Save'}
            </Button>
