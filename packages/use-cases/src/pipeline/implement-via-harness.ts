...
/**
 * M5.7 — Bridge the Pipeline's `implementation` phase into the Harness.
 *
 * The flow:
 ...
 *      `specEdits` (M5.5) when present, otherwise fall back to the
...
export class ImplementViaHarnessUseCase {
  ...
  private async createHarnessProject(
    project: PipelineProject,
    specPath: string,
  ) {
  ...
  private async planHarnessSprints(harnessProjectId: string, specContent: string): Promise<HarnessSprint[]> {
