...
export function DiffViewer({ before, after, title, maxLines = 1000 }: DiffViewerProps) {
  ...
  return (
    <div className="bg-muted/40 overflow-hidden rounded-md border font-mono text-xs">
      <div className="bg-muted text-muted-foreground flex items-center justify-between border-b px-3 py-1.5 text-xs uppercase tracking-wide">
          ...
          <tbody>
            {visible.map((op, idx) => (
              <tr key={idx} className={rowClass(op.type)}>
                <td className="text-muted-foreground w-10 select-none border-r px-2 py-0.5 text-right text-xs">
                  {op.oldLine ?? ''}
                </td>
                <td className="text-muted-foreground w-10 select-none border-r px-2 py-0.5 text-right text-xs">
                  {op.newLine ?? ''}
                </td>
                <td className="text-muted-foreground w-3 select-none px-2 py-0.5 text-xs">
                  {PREFIX[op.type]}
                </td>
                <td className="whitespace-pre px-2 py-0.5">{op.text || '\u00A0'}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {truncated && (
          <div className="bg-muted text-muted-foreground border-t px-3 py-1 text-xs">
