...
const STATE_STYLES: Record<VoiceConversationState, string> = {
  idle: 'bg-gray-200 hover:bg-gray-300 scale-100',
  listening: 'bg-blue-400 scale-110 animate-pulse',
  processing: 'bg-yellow-400 scale-105 animate-spin',
  speaking: 'bg-green-400 scale-110',
};
...
export function VoiceOrb({ state, onClick }: VoiceOrbProps) {
  return (
    <button
      ...
      className={`h-16 w-16 rounded-full transition-all duration-300 shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-400 ${STATE_STYLES[state]}`}
    >
      <span className="sr-only">{STATE_ARIA[state]}</span>
      {state === 'listening' && (
        <svg viewBox="0 0 24 24" className="h-8 w-8 mx-auto text-white" fill="currentColor">
      ...
      {(state === 'idle' || state === 'processing') && (
        <svg viewBox="0 0 24 24" className="h-8 w-8 mx-auto text-gray-600" fill="currentColor">
