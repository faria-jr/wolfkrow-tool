...
function log(lvl: 'warn' | 'error' | 'info', name: string, msg: string, extra?: unknown): void {
  const entry = { time: new Date().toISOString(), level: lvl, service: 'wolfkrow-wrapper', name, msg, ...(extra !== undefined ? { extra } : {}) };
  const line = isDev ? `[${lvl.toUpperCase()}] [${name}] ${msg}${extra !== undefined ? ` ${JSON.stringify(extra)}` : ''}` : JSON.stringify(entry);
