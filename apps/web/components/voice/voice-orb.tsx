...
export function VoiceOrb({ state, onClick }: VoiceOrbProps) {
  return (
    <button
      ...
      className={`h-16 w-16 rounded-full transition-all duration-300 shadow-lg focus:outline-none focus:ring-2 focus:ring-info ${STATE_STYLES[state]}`}
    >
      <span className="sr-only">{STATE_ARIA[state]}</span>
      {state === 'listening' && (
        <svg viewBox="0 0 24 24" className="h-8 w-8 mx-auto text-info-foreground" fill="currentColor">
      ...
      {state === 'speaking' && (
        <svg viewBox="0 0 24 24" className="h-8 w-8 mx-auto text-success-foreground" fill="currentColor">
      ...
      {(state === 'idle' || state === 'processing') && (
        <svg viewBox="0 0 24 24" className="h-8 w-8 mx-auto text-muted-foreground" fill="currentColor">
