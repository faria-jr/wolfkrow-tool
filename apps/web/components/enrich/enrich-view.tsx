...
export function EnrichView() {
  ...
  const handleValidate = async (id: string) => {
    setActionId(id);
    await apiPost(`/api/enrich/sessions/${id}/validate`);
    await load();
    setActionId(null);
  };
  ...
  const handleEnrich = async (id: string) => {
    setActionId(id);
    await apiPost(`/api/enrich/sessions/${id}/enrich`);
    await load();
    setActionId(null);
  };
    ...
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      ...
      {sessions.length === 0 ? (
        ...
        <ul className="space-y-3">
          {sessions.map((s) => (
            <li key={s.id} className="bg-card flex items-center justify-between gap-4 rounded-lg border p-4">
              ...
              <div className="flex shrink-0 gap-2">
                <button
                  onClick={() => void handleValidate(s.id)}
                ...
                <button
                  onClick={() => void handleEnrich(s.id)}
                  disabled={actionId === s.id}
