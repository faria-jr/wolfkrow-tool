...
export function registerFinalizeRoutes(app: Express, ctx: RegisterFinalizeRoutesDeps) {
  ...
  // SPEC docs/spec-opendesign-vendor.md secao 5.11 (linhas 905-973, 942-944):
  // quando `OD_EMBED_HOST=lionclaw` esta no env do sidecar (injetado pelo
  // LionClaw `manager.start`), bloquear server-side qualquer rota
  // `*/finalize/*` com HTTP 403. Defense-in-depth contra DevTools / replay /
  // qualquer caminho que driblar a UI escondida. Patch CONDICIONAL: sem a
  // env, o daemon mantem o comportamento upstream.
  ...
  app.post('/api/projects/:id/finalize/anthropic', async (req, res) => {
    // LionClaw embed-mode (defense-in-depth, vendor/.lionclaw-patches/embed-mode.md).
    // Mesmo guard que o middleware acima, repetido aqui para garantir bloqueio
    // mesmo se a ordem de registro do middleware mudar no futuro. SPEC 5.11.
