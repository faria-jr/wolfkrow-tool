...
export interface DetachSkillInput {
  skillId: string;
  agentId: string;
}
export interface DetachSkillOutput {
  agent: Agent;
}
...
export class DetachSkillFromAgentUseCase implements UseCase<DetachSkillInput, DetachSkillOutput> {
  ...
  async execute(input: DetachSkillInput): Promise<DetachSkillOutput> {
    ...
    const updated = await this.agentRepo.save(
      agent.update({ skills: agent.skills.filter((s) => s !== input.skillId) })
    );
