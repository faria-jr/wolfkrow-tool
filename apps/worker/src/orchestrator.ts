...
import { KEYTAR_SERVICE } from './lib/keychain';
...
export class OrchestratorService {
  ...
  constructor(options: OrchestratorOptions = {}) {
    ...
    this.factory =
      options.factory ??
      new ProviderAIProviderFactory(getToolRegistry(), defaultPermissionResolver);
  ...
  private async resolveProvider(request: ChatRequest): Promise<AIProvider> {
    const resolved = await resolveAIProvider({
      ...
      keytarService: this.keytarService,
    });
    this.logger?.info(
      {
        provider: resolved.config.id,
        model: request.model,
        apiKeyAccount: resolved.config.apiKeyAccount,
      },
      'Orchestrator: resolving provider'
    );
