...
describe('google-calendar MCP server smoke tests', () => {
  ...
  it('calendar_list_events calls Google Calendar API and returns content', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        items: [
          { id: 'evt1', summary: 'Team Meeting', start: { dateTime: '2026-06-23T10:00:00Z' }, end: { dateTime: '2026-06-23T11:00:00Z' } },
        ],
      }),
    }));
    ...
    const res = await handleRpcMessage(
      { jsonrpc: '2.0', id: 3, method: 'tools/call', params: { name: 'calendar_list_events', arguments: { maxResults: 5 } } },
      handlers,
    );
    ...
    const res = await handleRpcMessage(
      {
        ...
        params: {
          ...
          arguments: { summary: 'New Meeting', startDateTime: '2026-06-24T14:00:00Z', endDateTime: '2026-06-24T15:00:00Z' },
        },
      },
      handlers,
    );
  ...
  it('missing token returns error content', async () => {
    ...
    const res = await handleRpcMessage(
      { jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'calendar_list_events', arguments: {} } },
      handlers,
    );
