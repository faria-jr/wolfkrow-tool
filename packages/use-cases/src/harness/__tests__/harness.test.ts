import type { HarnessProjectRepo, HarnessRoundRepo, HarnessSprintRepo, SprintFeature } from '@wolfkrow/domain';
...
class InMemoryProjectRepo implements HarnessProjectRepo {
  ...
  async findByUserId(userId: string) { return [...this.store.values()].filter((p) => p.userId === userId); }
...
class InMemorySprintRepo implements HarnessSprintRepo {
  ...
  async findById(id: string) { return this.store.get(id) ?? null; }
  async findByProjectId(projectId: string) { return [...this.store.values()].filter((s) => s.projectId === projectId); }
...
class InMemoryRoundRepo implements HarnessRoundRepo {
  ...
  async findById(id: string) { return this.store.get(id) ?? null; }
  async findBySprintId(sprintId: string) { return [...this.store.values()].filter((r) => r.sprintId === sprintId); }
  async findBySprintAndFeature(sprintId: string, featureIndex: number) {
    return [...this.store.values()].filter((r) => r.sprintId === sprintId && r.featureIndex === featureIndex);
  }
  async save(r: HarnessRound) { this.store.set(r.id, r); return r; }
}

const FEATURES: SprintFeature[] = [
  { name: 'Login', description: 'User can log in', acceptanceCriteria: ['POST /auth/login returns JWT'] },
];
...
describe('GetHarnessProjectUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    const p = HarnessProject.create({ userId: 'u1', name: 'P', specPath: '/s', description: undefined, config: { maxRoundsPerFeature: 5, coderModel: 'x', plannerModel: 'y' } });
    await repo.save(p);
    projectId = p.id;
  });
...
describe('ListHarnessProjectsUseCase', () => {
  it('lists projects for user', async () => {
    const repo = new InMemoryProjectRepo();
    await repo.save(HarnessProject.create({ userId: 'u1', name: 'A', specPath: '/a', description: undefined, config: { maxRoundsPerFeature: 5, coderModel: 'x', plannerModel: 'y' } }));
    await repo.save(HarnessProject.create({ userId: 'u2', name: 'B', specPath: '/b', description: undefined, config: { maxRoundsPerFeature: 5, coderModel: 'x', plannerModel: 'y' } }));
...
describe('DeleteHarnessProjectUseCase', () => {
  it('deletes project', async () => {
    const repo = new InMemoryProjectRepo();
    const p = HarnessProject.create({ userId: 'u1', name: 'A', specPath: '/a', description: undefined, config: { maxRoundsPerFeature: 5, coderModel: 'x', plannerModel: 'y' } });
    await repo.save(p);
...
describe('PlanSprintsUseCase', () => {
  it('creates sprints from planner output', async () => {
    ...
    const mockPlanner = { plan: vi.fn().mockResolvedValue([{ name: 'Sprint 1', description: 'Auth sprint', features: FEATURES }]) };
...
describe('RunCoderRoundUseCase', () => {
  it('creates round with coder output', async () => {
    ...
    const sprint = HarnessSprint.create({ projectId: 'p1', number: 1, name: 'S1', description: undefined, features: FEATURES });
    await sprintRepo.save(sprint);

    const mockCoder = { implement: vi.fn().mockResolvedValue({ output: 'code here', tokens: 1000 }) };
...
describe('EvaluateRoundUseCase', () => {
  it('marks round as passed when evaluator approves', async () => {
    ...
    const mockEvaluator = { evaluate: vi.fn().mockResolvedValue({ passed: true, feedback: 'Looks good!', tokens: 200 }) };
  ...
  it('marks round as failed when evaluator rejects', async () => {
    ...
    const mockEvaluator = { evaluate: vi.fn().mockResolvedValue({ passed: false, feedback: 'Tests fail', tokens: 150 }) };
    const uc = new EvaluateRoundUseCase(roundRepo, mockEvaluator);
    const result = await uc.execute({ roundId: withOutput.id });
