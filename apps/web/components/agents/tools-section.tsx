      ...
      <div className="flex flex-wrap gap-1">
        {PRESET_TOOLS.map((tool) => (
          <Button
            ...
            variant="outline"
            size="sm"
            onClick={() => addPreset(tool)}
            disabled={value.includes(tool)}
          >
            + {tool}
          </Button>
        ))}
...
export function ToolsSection({ control }: Props) {
  return (
    <div className="space-y-4">
      <h3 className="text-muted-foreground text-sm font-medium">Tools</h3>
      <FormField
        control={control}
        name="allowedTools"
        render={({ field }) => (
          <FormItem>
            ...
            <FormDescription className="text-xs">
              Tools this agent can use. Click badge to remove.
            </FormDescription>
