...
function mockJson(url: string, method: string, body: unknown, status = 200): typeof fetch {
  return vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (u.includes(url) && (!init?.method || init.method === method)) {
      return { ok: status >= 200 && status < 300, status, json: async () => body, text: async () => String(body) } as Response;
    }
...
describe('OpenDesignClient', () => {
  ...
  it('POSTs /api/projects with the create body and returns project+conversationId', async () => {
    global.fetch = mockJson('/api/projects', 'POST', {
      conversationId: 'c1',
      project: { id: 'p1', name: 'WK', skillId: null, designSystemId: null, metadata: { kind: 'prototype', fidelity: 'high-fidelity', source: 'wolfkrow' }, createdAt: 1, updatedAt: 1 },
    });
    const c = new OpenDesignClient('http://127.0.0.1:7456');
    const r = await c.createProject({ id: 'p1', name: 'WK', metadata: { kind: 'prototype', fidelity: 'high-fidelity', source: 'wolfkrow' } });
  ...
  it('fetches a file as text', async () => {
    global.fetch = vi.fn(async () => ({ ok: true, status: 200, text: async () => '<html></html>', json: async () => ({}) }) as Response) as unknown as typeof fetch;
    const c = new OpenDesignClient('http://127.0.0.1:7456');
    expect(await c.getProjectFile('p1', 'index.html')).toBe('<html></html>');
    expect(vi.mocked(global.fetch).mock.calls[0]?.[0]).toContain('/api/projects/p1/files/index.html');
  });

  it('throws on non-ok response', async () => {
    global.fetch = vi.fn(async () => ({ ok: false, status: 500, json: async () => ({}), text: async () => '' }) as Response) as unknown as typeof fetch;
    const c = new OpenDesignClient('http://127.0.0.1:7456');
