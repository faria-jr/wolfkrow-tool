...
export class SendMessageUseCase implements UseCase<SendMessageInput, SendMessageOutput> {
  ...
  private async *streamAndSave(
    ...
    try {
      ...
      // Stop button aborts mid-stream — flush the partial message instead of losing it.
  ...
  private async finalizeTurn(ctx: {
    ...
    if (accumulated) {
      const assistantMsg = MessageEntity.create({
        sessionId: session.id,
        userId: input.userId,
        role: 'assistant',
        content: accumulated,
...
function isAbortError(err: unknown): boolean {
  ...
  return (
    typeof DOMException !== 'undefined' && err instanceof DOMException && /abort/i.test(err.name)
  );
}
