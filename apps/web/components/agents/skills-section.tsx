  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1">
        {value.map((skill) => (
          <Badge key={skill} variant="outline" className="cursor-pointer gap-1" onClick={() => remove(skill)}>{skill} ×</Badge>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <FormField control={control} name="skills" render={({ field }) => (
        <FormItem>
          ...
          <FormDescription className="text-xs">Capabilities injected into system prompt. Click badge to remove.</FormDescription>
