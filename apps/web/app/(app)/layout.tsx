...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SidebarProvider defaultOpen>
      <a
        href="#main-content"
        className="focus:bg-background sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-50 focus:rounded-md focus:px-4 focus:py-2 focus:shadow"
      >
        Skip to content
      </a>
      <AutoLock />
      ...
      <SidebarInset>
        ...
        <div id="main-content" tabIndex={-1} className="min-h-0 flex-1 overflow-hidden">
          {children}
        </div>
