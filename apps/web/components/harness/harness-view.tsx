...
function ProjectListItem({ project: p, isSelected, onSelect, onPlan, onDelete, planning }: ListItemProps) {
  return (
    <li className={`cursor-pointer rounded border p-3 ${isSelected ? 'border-blue-500 bg-blue-50' : 'hover:bg-gray-50'}`} onClick={onSelect}>
      ...
      <div className="mt-2 flex gap-2">
        <button onClick={(e) => { e.stopPropagation(); onPlan(); }} disabled={planning} className="rounded bg-purple-600 px-2 py-1 text-xs text-white hover:bg-purple-700 disabled:opacity-50">
          {planning ? 'Planning…' : 'Plan Sprints'}
        </button>
        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="rounded border border-red-300 px-2 py-1 text-xs text-red-600 hover:bg-red-50">
          Delete
        </button>
      </div>
    </li>
