...
import {
  NotFoundError,
  PipelineMessage as PipelineMessageEntity,
...
const PHASE_PROMPTS: Partial<Record<PipelineStage, string>> = {
  discovery:
    'You are a product discovery expert. Analyze the project and produce structured discovery notes: problem statement, target users, key requirements, constraints, and success metrics. Be concise and actionable.',
  spec_build:
    'You are a technical architect. Based on the discovery notes, build a detailed technical specification: architecture decisions, API contracts, data models, implementation strategy, and risk assessment.',
  spec_validate:
    'You are a senior QA engineer. Validate the technical specification for completeness, consistency, and feasibility. List any gaps, contradictions, or risks found. Provide a validation summary.',
  implementation:
    'You are a senior software engineer. Implement the specified features following the technical specification. Write clean, tested, production-ready code.',
};
...
export class RunPhaseUseCase {
  ...
  async execute(input: RunPhaseInput): Promise<RunPhaseOutput> {
    ...
    const cost = defaultPricingCalculator.cost(
      input.model ?? 'claude-sonnet-4-6',
      result.usage
    ).usdCents;
    const artifactPath = await this.persistExchange(project, phase, userContent, result);

    const completedPhase = await this.phaseRepo.save(
      phase.complete(artifactPath, tokens, undefined, cost)
    );

    const nextStage = NEXT_STAGE[phase.stage];
    const updatedProject = await this.projectRepo.save(
      nextStage ? project.withStage(nextStage) : project.withStatus('completed', new Date())
    );
