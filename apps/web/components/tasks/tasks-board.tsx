      ...
      {showForm ? (
        <div className="flex items-center gap-2">
          <Input
            ...
            className="flex-1"
          />
          <Button onClick={() => void handleCreate()} disabled={saving || !title.trim()}>Add</Button>
          ...
          return (
            <div key={key} className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-secondary px-2 py-0.5 text-xs">{col.length}</span>
              </div>
              <div className="flex flex-col gap-2">
                {col.map((task) => (
                  <div
                    key={task.id}
                    className={`rounded border border-l-4 bg-card p-2 ${PRIORITY_COLORS[task.priority]}`}
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      {STATUS_COLS.filter((s) => s.key !== task.status).map((s) => (
                        <button
                          key={s.key}
                          onClick={() => void handleMove(task.id, s.key)}
